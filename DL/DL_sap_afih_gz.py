...
# USERNAME = 'MITRATEL094'
# PASSWORD = 'Mitratelnumber1!'
...
def choose_fetch_mode(**kwargs):
    ...
    if os.path.exists(SKIP_TOKEN_PATH):
        return 'fetch_gz_initial'
    elif os.path.exists(DELTA_LINK_PATH):
        return 'fetch_gz_delta'
    else:
        return 'fetch_gz_initial'
...
def fetch_skip_token_url():
    try:
        ...
        if os.path.exists(SKIP_TOKEN_PATH):
            with open(SKIP_TOKEN_PATH, "r") as f:
                skiptoken = f.read().strip()
            return NEXT_URL + skiptoken
        else:
            return BASE_URL + '&InitialLoad=true'
    except Exception as e:
        raise    

def fetch_gz_initial():
    url = fetch_skip_token_url()
    ...
    if os.path.exists(SKIP_TOKEN_PATH):
        ...
        --insecure \
        --header "Accept-Encoding: gzip" \
        --header "Prefer: odata.track-changes,odata.maxpagesize=50000" \
        --output "{XML_DIR}/batch_0.xml.gz"
        """

def decompress_gz_file(**kwargs):
    gz_path = os.path.join(XML_DIR, 'batch_0.xml.gz')
    xml_path = os.path.join(XML_DIR, 'batch_0.xml')
    try:
...
def fetch_gz_delta_initial():
    pattern = os.path.join(XML_DIR, 'delta*.xml*')
    for file_path in glob.glob(pattern):
        os.remove(file_path)

    url = DELTA_DISCOVERY_URL
    auth_header = get_basic_auth_header()
    return f"""
    curl --location "{url}" \
    --header "{auth_header}" \
    --insecure \
    --header "Accept-Encoding: gzip" \
    --output "{XML_DIR}/delta_0.xml.gz"
    """

def decompress_gz_file_delta_initial(**kwargs):
    gz_path = os.path.join(XML_DIR, 'delta_0.xml.gz')
    xml_path = os.path.join(XML_DIR, 'delta_0.xml')
    try:
        with gzip.open(gz_path, 'rb') as f_in:
            with open(xml_path, 'wb') as f_out:
                shutil.copyfileobj(f_in, f_out)
...
def fetch_skip_token_url_delta():
    try:
        pattern = os.path.join(XML_DIR, 'delta*.xml*')
        for file_path in glob.glob(pattern):
            os.remove(file_path)
        ...
        if os.path.exists(DELTA_LINK_PATH):
            with open(DELTA_LINK_PATH, 'r') as f:
                return NEXT_URL + f.read()
        else:
            raise Exception("Delta link file not found. Please run the initial fetch first.")
    except Exception as e:
        raise    

def fetch_gz_delta():
    url = fetch_skip_token_url_delta()
    auth_header = get_basic_auth_header()
    return f"""
    curl --location "{url}" \
    --header "{auth_header}" \
...
def decompress_gz_file_delta(**kwargs):
    gz_path = os.path.join(XML_DIR, 'delta_0.xml.gz')
    xml_path = os.path.join(XML_DIR, 'delta_0.xml')
    try:
        with gzip.open(gz_path, 'rb') as f_in:
...
with DAG(
    ...
    curl_gz = BashOperator(
        task_id='curl_odata_gz',
        bash_command="{{ task_instance.xcom_pull(task_ids='fetch_gz_initial') }}",
        do_xcom_push=False
    )

    py_fetch_gz_initial = PythonOperator(
        task_id='fetch_gz_initial',
        python_callable=fetch_gz_initial
    )

    decompress = PythonOperator(
        task_id='decompress_gz',
        python_callable=decompress_gz_file
    )

    curl_gz_delta_initial = BashOperator(
        task_id='curl_odata_gz_delta_initial',
        bash_command="{{ task_instance.xcom_pull(task_ids='fetch_gz_delta_initial') }}",
        do_xcom_push=False
    )

    py_fetch_gz_delta_initial = PythonOperator(
        task_id='fetch_gz_delta_initial',
        python_callable=fetch_gz_delta_initial
    )

    decompress_delta_initial = PythonOperator(
        task_id='decompress_gz_delta_initial',
        python_callable=decompress_gz_file_delta_initial
    )
    ...
    curl_gz_delta = BashOperator(
        task_id='curl_odata_gz_delta',
        bash_command="{{ task_instance.xcom_pull(task_ids='fetch_gz_delta') }}",
        do_xcom_push=False
    )

    py_fetch_gz_delta = PythonOperator(
        task_id='fetch_gz_delta',
        python_callable=fetch_gz_delta
    )

    decompress_delta = PythonOperator(
        task_id='decompress_gz_delta',
        python_callable=decompress_gz_file_delta
    )
    ...
    branch >> py_fetch_gz_initial >> curl_gz >> decompress >> initial_task >> py_fetch_gz_delta_initial >> curl_gz_delta_initial >> decompress_delta_initial >> store_delta_link >> parse_task_init
    branch >> py_fetch_gz_delta >> curl_gz_delta >> decompress_delta >> fetch_delta_task >> parse_task
