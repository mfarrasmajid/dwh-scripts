...
# Constants
BASE_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AFVC_2_SRV/FactsOfZCDCAFVC?sap-client=300'
DELTA_DISCOVERY_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AFVC_2_SRV/DeltaLinksOfFactsOfZCDCAFVC?sap-client=300'
NEXT_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AFVC_2_SRV/'
HEADERS = {
...
DELTA_LINK_PATH = '/tmp/sap_afvc_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sap_afvc_skip_token.txt'
XML_DIR = '/tmp/sap_afvc'
DAG_ID = "DL_sap_afvc"
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                aufpl = r['AUFPL']
                aplzl = r['APLZL']
                DELETE_QUERY =  f"ALTER TABLE `sap`.`afvc` DELETE WHERE AUFPL = '{aufpl}' AND APLZL = '{aplzl}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
