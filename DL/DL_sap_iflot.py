...
# Constants
BASE_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_IFLOT_1_SRV/FactsOfZCDCIFLOT?sap-client=300'
...
DELTA_LINK_PATH = '/tmp/sap_iflot_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sap_iflot_skip_token.txt'
XML_DIR = '/tmp/sap_iflot'
DAG_ID = "DL_sap_iflot"
...
def load_to_clickhouse(**kwargs):
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                tplnr = r['TPLNR']
                DELETE_QUERY =  f"ALTER TABLE `sap`.`iflot` DELETE  WHERE TPLNR = '{tplnr}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
