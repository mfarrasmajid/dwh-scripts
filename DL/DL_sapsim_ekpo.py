...
# Constants
BASE_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_EKPO_1_SRV/FactsOfZCDCEKPO?sap-client=300'
DELTA_DISCOVERY_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_EKPO_1_SRV/DeltaLinksOfFactsOfZCDCEKPO?sap-client=300'
NEXT_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_EKPO_1_SRV/'
HEADERS = {
...
DELTA_LINK_PATH = '/tmp/sapsim_ekpo_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sapsim_ekpo_skip_token.txt'
XML_DIR = '/tmp/sapsim_ekpo'
DAG_ID = "DL_sapsim_ekpo"
DAG_INTERVAL = "3-59/5 0-6,8-23 * * *"
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                ...
                ebelp = r['EBELP']
                uniqueid = r['UNIQUEID']
                DELETE_QUERY =  f"ALTER TABLE `sapsim`.`ekpo` DELETE WHERE EBELN = '{ebeln}' AND EBELP = '{ebelp}' AND UNIQUEID = '{uniqueid}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
