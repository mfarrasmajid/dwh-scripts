...
# Constants
BASE_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_FMZUOB_1_SRV/FactsOfZCDCFMZUOB?sap-client=300'
...
DELTA_LINK_PATH = '/tmp/sapsim_fmzuob_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sapsim_fmzuob_skip_token.txt'
XML_DIR = '/tmp/sapsim_fmzuob'
DAG_ID = "DL_sapsim_fmzuob"
DAG_INTERVAL = "4-59/5 17-22 * * *"
...
TAGS = ["dl", "sapsim", "fmzuob"]
INSERT_QUERY = """
                    INSERT INTO `sapsim`.`fmzuob`
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                ...
                abper = r['ABPER']
                abgjr = r['ABGJR']
                DELETE_QUERY =  f"ALTER TABLE `sapsim`.`fmzuob` DELETE WHERE OBJNR = '{objnr}' AND KSTAR = '{kstar}' AND ABPER = '{abper}' AND ABGJR = '{abgjr}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
