...
# Constants
BASE_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_ZDMTFITB040_1_SRV/FactsOfZCDCZDMTFITB040?sap-client=300'
DELTA_DISCOVERY_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_ZDMTFITB040_1_SRV/DeltaLinksOfFactsOfZCDCZDMTFITB040?sap-client=300'
NEXT_URL = 'http://dmterpsim.mitratel.co.id:8041/sap/opu/odata/sap/ZCDC_ZDMTFITB040_1_SRV/'
HEADERS = {
...
DELTA_LINK_PATH = '/tmp/sapsim_zdmt_fitb040_delta_link.txt'
START_TOKEN_PATH = '/tmp/sapsim_zdmt_fitb040_start_token.txt'
SKIP_TOKEN_PATH = '/tmp/sapsim_zdmt_fitb040_skip_token.txt'
XML_DIR = '/tmp/sapsim_zdmt_fitb040'
DAG_ID = "DL_sapsim_zdmt_fitb040"
DAG_INTERVAL = "10-59/15 * * * *"
CLICKHOUSE_CONN_ID = "clickhouse_mitratel"
CLICKHOUSE_DATABASE = "sapsim"
CLICKHOUSE_TABLE = "zdmt_fitb040"
LOG_CONN_ID = "airflow_logs_mitratel"
LOG_TABLE = "airflow_logs"
LOG_TYPE = "delta"
LOG_KATEGORI = "Data Lake" 
TAGS = ["dl", "sapsim", "zdmt_fitb040"]
# INSERT_QUERY = """
#                     INSERT INTO `sapsim`.`zdmt_fitb040`
#                     (
#                     `INDEXNO`, `FUNDCENTER`, `CMMNTITEM`, `PRNO`, `PRITEM`, `PRCREATE`, `PRREQDATE`,
#                     `PRRELDATE`, `PRDELIVDATE`, `PRTRACKNO`, `PRQTY`, `PRMEINS`, `PRVALUE`,
#                     `PRWAERS`, `PONO`, `POITEM`, `POCRDATE`, `PODOCDATE`, `POVALUE`, `ELIKZ`, `POWAERS`,
#                     `VENDOR`, `VENDORNAME`, `GRDOC`, `GRYEAR`, `GRQTY`, `GRUOM`, `GRCRDT`, `GRDCDT`,
#                     `GRBUDT`, `GRACCDOC`, `GRACCYEAR`, `GRACCVALUE`, `GRACCWAERS`, `INVDOC`, `INVYEAR`,
#                     `INVXBLNR`, `INVCRDT`, `INVDCDT`, `INVBUDT`, `INVACCDOC`, `INVACCYEAR`, `SGTXT`,
#                     `INVVALUE`, `INVWAERS`, `INVACCAUGBL`, `INVACCAUGJ`, `INVACCAUGDT`, `RSNUM`,
#                     `RSPOS`, `ODQ_CHANGEMODE`, `ODQ_ENTITYCNTR`)
#                     VALUES
#                     (
#                     %(INDEXNO)s, %(FUNDCENTER)s, %(CMMNTITEM)s, %(PRNO)s, %(PRITEM)s, %(PRCREATE)s, 
#                     %(PRREQDATE)s, %(PRRELDATE)s, %(PRDELIVDATE)s, %(PRTRACKNO)s, %(PRQTY)s, %(PRMEINS)s, 
#                     %(PRVALUE)s, %(PRWAERS)s, %(PONO)s, %(POITEM)s, %(POCRDATE)s, %(PODOCDATE)s, %(POVALUE)s, 
#                     %(ELIKZ)s, %(POWAERS)s, %(VENDOR)s, %(VENDORNAME)s, %(GRDOC)s, %(GRYEAR)s, %(GRQTY)s, 
#                     %(GRUOM)s, %(GRCRDT)s, %(GRDCDT)s, %(GRBUDT)s, %(GRACCDOC)s, %(GRACCYEAR)s, %(GRACCVALUE)s, 
#                     %(GRACCWAERS)s, %(INVDOC)s, %(INVYEAR)s, %(INVXBLNR)s, %(INVCRDT)s, %(INVDCDT)s, %(INVBUDT)s, 
#                     %(INVACCDOC)s, %(INVACCYEAR)s, %(SGTXT)s, %(INVVALUE)s, %(INVWAERS)s, %(INVACCAUGBL)s, 
#                     %(INVACCAUGJ)s, %(INVACCAUGDT)s, %(RSNUM)s, %(RSPOS)s, %(ODQ_CHANGEMODE)s, %(ODQ_ENTITYCNTR)s)
#                 """
INSERT_QUERY = """
                    INSERT INTO `sapsim`.`zdmt_fitb040`
                    (
...
def choose_fetch_mode(**kwargs):
    ...
    try:
        ...
        raise
    return 'fetch_sap_cdc_initial' if os.path.exists(START_TOKEN_PATH) else 'finish_dag'
    # return 'fetch_sap_delta' if os.path.exists(DELTA_LINK_PATH) else 'fetch_sap_cdc_initial'
    ...
    try:
        ...
        if next_link_el is not None:
            ...
            if skiptoken:
                ...
                skiptoken_url = f"FactsOfZCDCZDMTFITB040?sap-client=300&$skiptoken={skiptoken}"
...
#     try:
#         response = requests.get(DELTA_DISCOVERY_URL, auth=HTTPBasicAuth(USERNAME, PASSWORD))
#         response.raise_for_status()
...
#         if not (changes_after_href and delta_token):
...
def load_to_clickhouse_init(**kwargs):
    ...
    try:
        ...
        #         indexno = r['INDEXNO']
        #         DELETE_QUERY =  f"ALTER TABLE `sap`.`zdmt_fitb040` DELETE WHERE INDEXNO = '{indexno}'"
...
def load_to_clickhouse(**kwargs):
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                indexno = r['INDEXNO']
                DELETE_QUERY =  f"ALTER TABLE `sapsim`.`zdmt_fitb040` DELETE WHERE INDEXNO = '{indexno}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
...
with DAG(
    ...
    branch >> initial_task >> parse_task_init
