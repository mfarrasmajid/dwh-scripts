...
# Constants
BASE_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AUFK_1_SRV/FactsOfZCDCAUFK?sap-client=300'
DELTA_DISCOVERY_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AUFK_1_SRV/DeltaLinksOfFactsOfZCDCAUFK?sap-client=300'
NEXT_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_AUFK_1_SRV/'
HEADERS = {
...
DELTA_LINK_PATH = '/tmp/sap_aufk_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sap_aufk_skip_token.txt'
XML_DIR = '/tmp/sap_aufk'
DAG_ID = "DL_sap_aufk"
DAG_INTERVAL = "*/3 * * * *"
...
def load_to_clickhouse(**kwargs):
    ...
    try:
        ...
        for r in records:
            if r['ODQ_CHANGEMODE'] == 'D':
                aufnr = r['AUFNR']
                DELETE_QUERY =  f"ALTER TABLE `sap`.`aufk` DELETE  WHERE AUFNR = '{aufnr}'"
                client.execute(DELETE_QUERY)
            else:
                client.execute(INSERT_QUERY, r)
        log_status(process_name, random_value, "success")
