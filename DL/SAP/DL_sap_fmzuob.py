...
# Constants
BASE_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_FMZUOB_1_SRV/FactsOfZCDCFMZUOB?sap-client=300'
DELTA_DISCOVERY_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_FMZUOB_1_SRV/DeltaLinksOfFactsOfZCDCFMZUOB?sap-client=300'
NEXT_URL = 'http://dmterpprd2.mitratel.co.id:8031/sap/opu/odata/sap/ZCDC_FMZUOB_1_SRV/'
HEADERS = {
...
DELTA_LINK_PATH = '/tmp/sap_fmzuob_delta_link.txt'
SKIP_TOKEN_PATH = '/tmp/sap_fmzuob_skip_token.txt'
XML_DIR = '/tmp/sap_fmzuob'
DAG_ID = "DL_sap_fmzuob"
DAG_INTERVAL = "2-59/3 * * * *"
CLICKHOUSE_CONN_ID = "clickhouse_mitratel"
CLICKHOUSE_DATABASE = "sap"
CLICKHOUSE_TABLE = "fmzuob"
...
INSERT_QUERY = """
                    ...
                    `GRANTNBR`,`FAREA`,`GRUPPEREF`,`GRFLDREF`,`KARGRREF`,`MEASURE`,`BUDGETPD`,`ODQ_CHANGEMODE`,
                    `ODQ_ENTITYCNTR`)
                    VALUES
                    (%(OBJNR)s,%(KSTAR)s,%(ABPER)s,%(ABGJR)s,%(KOKRS)s,%(FIKRS)s,%(FIPOS)s,%(FISTL)s,%(FONDS)s,
                    %(GRANTNBR)s,%(FAREA)s,%(GRUPPEREF)s,%(GRFLDREF)s,%(KARGRREF)s,%(MEASURE)s,%(BUDGETPD)s,%(ODQ_CHANGEMODE)s,
                    %(ODQ_ENTITYCNTR)s)
                """
INSERT_QUERY_2 = """
                    INSERT INTO `sap`.`fmzuob`
...
def sanitize_record(record):
    for key in ['OBJNR', 'KSTAR', 'ABPER', 'ABGJR']:
    ...
    try:
        ...
        if next_link_el is not None:
            ...
            if skiptoken:
                ...
                skiptoken_url = f"FactsOfZCDCFMZUOB?sap-client=300&$skiptoken={skiptoken}"
