...
dag = DAG(
    dag_id='DL_supof_generate_supof_table',
    default_args=default_args,
    schedule_interval='0 17 * * *',   # 17:00 Asia/Jakarta karena start_date bertz
    catchup=False,
    tags=['DL', 'support_oneflux'],
)
...
def mv_name_for(db: str, tbl: str) -> str:
    return f"_mv__{sanitize_name(db)}__{sanitize_name(tbl)}__from_raw"

# ===== CALLABLE =====
def build_all_supof_tables():
    # Ambil Variables di dalam callable agar parsing DAG tidak pecah bila belum ada
    ...
    RAW_DB_TABLE = "support_oneflux.all_mysql_data"
    MAX_KEYS_PER_TABLE = 5000  # hati-hati jumlah kolom
    ...
    print(f"Found {len(groups)} (db,table) groups in raw")

    for source_db, source_tbl, keys in groups:
        tgt_db = sanitize_name(source_db, append_underscore=True)
        tgt_tbl = sanitize_name(source_tbl)
        mv_name = mv_name_for(source_db, source_tbl)

        # 2) Pastikan DB target ada
        ...
        # 4) Buat tabel target kalau belum ada
        if not table_exists:
        ...
        # 5) Tambah kolom baru jika ada key baru
        new_cols = []
        for k in keys:
            if not k or not isinstance(k, str):
                continue
        ...
        SELECT
            {', '.join(select_cols)}
        FROM {RAW_DB_TABLE}
        WHERE source_database = %(sdb)s AND source_table = %(stbl)s
        """

        if not mv_exists:
            client.execute(base_create_mv.format(if_not_exists="IF NOT EXISTS"),
                           {"sdb": source_db, "stbl": source_tbl})
            print(f"Created MV {tgt_db}.{mv_name} -> {tgt_db}.{tgt_tbl}")
        else:
            if new_cols:
                client.execute(f"DROP VIEW {qident(tgt_db)}.{qident(mv_name)}")
                client.execute(base_create_mv.format(if_not_exists=""),
                               {"sdb": source_db, "stbl": source_tbl})
                print(f"Recreated MV {tgt_db}.{mv_name} with new columns")
            else:
                print(f"MV {tgt_db}.{mv_name} already up-to-date")

        # 7) Backfill jika tabel target kosong
...
generate_task = PythonOperator(
    task_id='generate_all_supof_table',
    python_callable=build_all_supof_tables,
    dag=dag,
)
