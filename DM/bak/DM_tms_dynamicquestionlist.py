...
SOURCE_TABLE = "dim_dynamicquestionlist"
TARGET_CONN_ID = "db_datamart_mitratel"
TARGET_DATABASE = "datamart"
TARGET_TABLE = "mart_tms_dynamicquestionlist"
FILE_PATH = "/tmp/debug_DM_tms_dynamicquestionlist.json"
DAG_ID = "DM_tms_dynamicquestionlist"
DAG_INTERVAL = "4-59/5 * * * *"
CHUNK_SIZE = 5000
...
TAGS = ["dm", "tms", "dynamicquestionlist"]

EXTRACT_QUERY = """
SELECT
    questionid, question, group as group_string, text_answer, number_answer, pict_answer, parent as reportid
FROM dwh_tms.dim_dynamicquestionlist FINAL
WHERE (group = 'Comment & Suggestion (Mengenai Tower)')
"""

SOURCE_COLUMNS=['questionid', 'question', 'group_string',
                'text_answer', 'number_answer', 'pict_answer', 'reportid']
...
def load_to_postgres(**kwargs):
    ...
    try:
        ...
        insert_query = """
        INSERT INTO public.mart_tms_dynamicquestionlist (
            questionid, question, group_string, text_answer, number_answer, pict_answer, reportid
        )
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (questionid) DO UPDATE 
        SET 
            question = EXCLUDED.question,
            group_string = EXCLUDED.group_string,
            text_answer = EXCLUDED.text_answer,
            number_answer = EXCLUDED.number_answer,
            pict_answer = EXCLUDED.pict_answer,
            reportid = EXCLUDED.reportid,
            updated_at = NOW() + INTERVAL '7 hours'
        """

        for __, row in df.iterrows():
            cursor.execute(insert_query, (row['questionid'], row['question'], row['group_string'], 
                                               row['text_answer'], row['number_answer'], row['pict_answer'], row['reportid']))
        conn.commit()
