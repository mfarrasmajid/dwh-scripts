...
SOURCE_TABLE = "fact_sitevisitrequest"
TARGET_CONN_ID = "db_datamart_mitratel"
TARGET_DATABASE = "datamart"
TARGET_TABLE = "mart_tms_visitrequest"
FILE_PATH = "/tmp/debug_DM_tms_visitrequest.json"
DAG_ID = "DM_tms_visitrequest"
DAG_INTERVAL = "4-59/5 * * * *"
CHUNK_SIZE = 5000
...
TAGS = ["dm", "tms", "visitrequest"]

EXTRACT_QUERY = """
SELECT visitrequest, creation, person_in_charge, pic_contact_mail, mobile_tms, role_unit, department_name
FROM dwh_tms.fact_sitevisitrequest FINAL
"""

SOURCE_COLUMNS=['visitrequest', 'creation', 'person_in_charge', 
                'pic_contact_mail', 'mobile_tms', 'role_unit', 'department_name']
...
def load_to_postgres(**kwargs):
    ...
    try:
        ...
        insert_query = """
        INSERT INTO public.mart_tms_visitrequest (
            visitrequest, creation, person_in_charge, pic_contact_mail, mobile_tms, role_unit, 
            department_name
        )
        VALUES (%s, %s, %s, %s, %s, %s, %s)
        ON CONFLICT (visitrequest, pic_contact_mail) DO UPDATE 
        SET 
        ...
        for __, row in df.iterrows():
            cursor.execute(insert_query, (row['visitrequest'], row['creation'], row['person_in_charge'], row['pic_contact_mail'], row['mobile_tms'], 
