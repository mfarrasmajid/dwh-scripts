...
TARGET_TABLE = "dim_alluser"
FILE_PATH = "/tmp/debug_DWH_tms_dimuser.json"
DAG_ID = "DWH_tms_dimuser"
DAG_INTERVAL = "2-59/15 * * * *"
CHUNK_SIZE = 5000
LOG_CONN_ID = "airflow_logs_mitratel"
LOG_TABLE = "airflow_logs"
LOG_TYPE = "truncate"
LOG_KATEGORI = "Data WareHouse"
TAGS = ["dwh", "tms", "dimuser"]

EXTRACT_QUERY = """
    SELECT DISTINCT name AS email,
        enabled,
        full_name,
        mobile_no,
        creation,
        ...
        customer_group,
        now() as updated_at
    FROM tms.tabuser FINAL
    """

SOURCE_COLUMNS=['email', 'enabled', 'full_name',
                'mobile_no', 'creation', 'last_login', 
                'company_name', 'module_profile', 'role_profile_name',
                'role_detail', 'role_unit', 'position', 'territory',
                'customer', 'customer_group', 'updated_at']
...
def extract_from_clickhouse(**kwargs):
    ...
    try:
        ...
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        data_new = [(
            row[0], row[1], row[2], row[3], clean_submit_date(row[4]), clean_submit_date(row[5]), row[6], row[7],
            row[8], row[9], row[10], row[11], row[12], row[13], row[14], now
        ) for row in data]
