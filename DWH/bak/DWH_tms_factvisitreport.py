...
TARGET_TABLE = "fact_sitevisitreport"
FILE_PATH = "/tmp/debug_DWH_tms_factvisitreport.json"
DAG_ID = "DWH_tms_factvisitreport"
DAG_INTERVAL = "2-59/5 * * * *"
CHUNK_SIZE = 5000
...
TAGS = ["dwh", "tms", "factvisitreport"]

EXTRACT_QUERY = """
    SELECT 
        a.name AS VisitReport, 
        a.email, 
        a.user_name, 
        a.unit_mtel, 
        a.direktorat_mtel, 
        ...
        b.province,
        now() as updated_at
    FROM tms.tabsitevisitreport a FINAL
    LEFT JOIN tms.tabsite b ON a.site_id = b.name
    """

SOURCE_COLUMNS=['VisitReport', 'email', 'user_name',
                'unit_mtel', 'direktorat_mtel', 'site_id', 
                'site_name', 'comment_tower', 'submit_date',
                'site_visit_requet', 'region', 'province',
                'updated_at']
...
def extract_from_clickhouse(**kwargs):
    ...
    try:
        ...
        now = datetime.now().strftime('%Y-%m-%d %H:%M:%S')
        data_new = [(
            row[0], row[1], row[2].replace("'", "") if row[2] else None, row[3], row[4], row[5], row[6], row[7],
            clean_submit_date(row[8]), row[9], row[10], row[11], now
        ) for row in data]
